...
import { getBills } from 'apiSdk/bills';
...
function DashboardPage() {
  ...
  const billStat = useSWR<PaginatedInterface<BillInterface>>(
    () => `/bills`,
    () => getBills({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Bills',
      result: billStat?.data?.totalCount,
      isLoading: billStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Products',
