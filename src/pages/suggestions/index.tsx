...
export function SuggestionListPage(props: SuggestionListPageProps) {
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('product', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('offer', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'offer',
          header: 'Offer',
          accessorKey: 'offer',
          cell: ({ row: { original: record } }: any) => (
            <Link as={NextLink} onClick={(e) => e.stopPropagation()} href={`/offers/view/${record.offer?.id}`}>
