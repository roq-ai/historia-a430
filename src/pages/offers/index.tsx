...
type ColumnType = ColumnDef<OfferInterface, unknown>;
...
export function OfferListPage(props: OfferListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getOffers({
        relations: ['product', 'suggestion.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<OfferInterface>>(
    () => `/offers?params=${JSON.stringify(params)}`,
    fetcher,
  );
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/offers/view/${record.id}`} passHref legacyBehavior>
                <Button
